...
This repo is the analytical backbone of the intraday trading research pipeline (origin: HFT microstructure). It characterizes every stock's microstructure -- spread dynamics, volatility regime, order flow patterns, and participant behavior -- across configurable timescales (1s to daily), producing the insights that drive feature extractor configuration and model selection for profitable intraday strategies (any of the pipeline's intraday approach classes).
